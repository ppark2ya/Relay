  ...
  return (
    <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-2 flex items-center gap-4">
      <div className="flex items-center gap-2">
        ...
        <span className="font-bold text-lg text-gray-800 dark:text-gray-100">Relay</span>
      </div>
      ...
      <div className="relative" ref={wsDropdownRef}>
        <button
          ...
          className="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-md text-xs flex items-center gap-2 hover:bg-gray-50 dark:hover:bg-gray-700 dark:text-gray-200"
        >
        ...
        {showWsDropdown && (
          <div className="absolute right-0 mt-1 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg dark:shadow-gray-900/50 z-10">
            <div className="py-1">
              {workspaces.length === 0 ? (
                <div className="px-3 py-2 text-xs text-gray-500 dark:text-gray-400">No workspaces</div>
              ) : (
                workspaces.map(ws => (
                  <button
                    key={ws.id}
                    onClick={() => { switchWorkspace(ws.id); setShowWsDropdown(false); }}
                    className={`w-full px-3 py-2 text-xs text-left hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2 dark:text-gray-200 ${ws.id === currentWorkspaceId ? 'bg-blue-50 dark:bg-blue-900/30' : ''}`}
                  >
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 py-1">
              <button
                ...
                className="w-full px-3 py-2 text-xs text-left hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2 text-blue-600 dark:text-blue-400"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="absolute right-0 mt-1 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg dark:shadow-gray-900/50 z-10">
            <div className="py-1">
              {environments.length === 0 ? (
                <div className="px-3 py-2 text-xs text-gray-500 dark:text-gray-400">No environments</div>
              ) : (
                environments.map(env => (
                  <button
                    key={env.id}
                    onClick={() => { activateEnv.mutate(env.id); setShowEnvDropdown(false); }}
                    className={`w-full px-3 py-2 text-xs text-left hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2 dark:text-gray-200 ${env.isActive ? 'bg-blue-50 dark:bg-blue-900/30' : ''}`}
                  >
          ...
          <div className="absolute right-0 mt-1 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg dark:shadow-gray-900/50 z-10">
            <div className="py-1">
              <button
                ...
                className={`w-full px-3 py-2 text-xs text-left hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2 ${!activeProxy ? 'bg-blue-50 dark:bg-blue-900/30' : ''}`}
              >
              ...
              {proxies.map(proxy => (
                <button
                  ...
                  className={`w-full px-3 py-2 text-xs text-left hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2 dark:text-gray-200 ${proxy.isActive ? 'bg-blue-50 dark:bg-blue-900/30' : ''}`}
                >
