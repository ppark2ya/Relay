...
// --- Helper: find the parent collection array and index for a given collection ID ---
function findCollectionSiblings(collections: Collection[], collectionId: number): { siblings: Collection[]; index: number; parentId: number | undefined } | undefined {
...
// --- Helper: find the parent collection array and index for a given request ID ---
function findRequestSiblings(collections: Collection[], requestId: number): { siblings: Request[]; index: number; collectionId: number } | undefined {
  for (const c of collections) {
    if (c.requests) {
...
export function Sidebar({ view, onViewChange, onSelectRequest, onSelectFlow, onSelectHistory, selectedRequestId, selectedFlowId }: SidebarProps) {
  ...
  // --- Flow DnD handler ---
  const handleFlowDragStart = (event: DragStartEvent) => {
      ...
      <div className="flex-1 overflow-y-auto p-2">
        ...
        {view === 'flows' && (
          <>
            ...
            <DndContext
              sensors={isDndDisabled ? undefined : flowSensors}
              collisionDetection={closestCenter}
              onDragStart={handleFlowDragStart}
